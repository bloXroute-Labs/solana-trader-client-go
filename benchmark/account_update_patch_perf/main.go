...
func main() {
	...
	flag.IntVar(&idleBeforeStart, "idle-before-start", 1, "how much to wait before analyzing the data")
	flag.Parse()

	var ch = make(chan accountUpdate, 1e7)
	var wg = new(sync.WaitGroup)
	var ctx, _ = context.WithTimeout(context.Background(), time.Duration(runDuration)*time.Minute+time.Duration(idleBeforeStart)*time.Minute)

	var idleEnd = time.Now().Add(time.Duration(idleBeforeStart) * time.Minute)

	fmt.Println("start idle, end at:", idleEnd.Format(time.TimeOnly))

	wg.Add(2)
	go accountUpdatesPatchLib(ctx, wg, ch, "127.0.0.1:9000", sourceAccountUpdateStream, idleEnd, false)
	go accountUpdatesPatchLib(ctx, wg, ch, "127.0.0.1:9001", sourceAccountUpdatePatchStream, idleEnd, false)
	...
	for upd := range ch {
		...
		if v, ok := mp[hash]; !ok {
			...
			// TODO: confirm logic for diff, but we are mainly interested in content compare
	...
	fmt.Println("compare pairs:", len(timeDiffsUs))

	fmt.Println("----------------")
	fmt.Println("values with (+) is AccountUpdatesStream, values with (-) is AccountUpdatesPatchStream")
	fmt.Println(calculatePercentiles(timeDiffsUs))
...
func accountUpdatesPatchLib(ctx context.Context, wg *sync.WaitGroup, ch chan accountUpdate, addr string, src source, idleEnd time.Time, testnet bool) {
	defer wg.Done()
	...
	for {
		...
		case <-ctx.Done():
			return
		case pool := <-chRaydium:
			fmt.Println("New Raydium pool:", pool)
			select {
