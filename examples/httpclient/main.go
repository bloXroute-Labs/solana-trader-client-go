...
func callOpenOrdersHTTP() {
	...
	opts := provider.DefaultRPCOpts(provider.MainnetSerumAPIHTTP, os.Getenv("AUTH_HEADER"))
...
func callPlaceOrderHTTP(ownerAddr, ooAddr string) uint64 {
	...
	rpcOpts := provider.DefaultRPCOpts(provider.MainnetSerumAPIHTTP, os.Getenv("AUTH_HEADER"))
