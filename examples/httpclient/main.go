...
func callOpenOrdersHTTP() {
	...
	opts := provider.DefaultRPCOpts(provider.MainnetSerumAPIHTTP, utils.AuthHeader)
...
func callPlaceOrderHTTP(ownerAddr, ooAddr string) uint64 {
	...
	rpcOpts := provider.DefaultRPCOpts(provider.MainnetSerumAPIHTTP, utils.AuthHeader)
